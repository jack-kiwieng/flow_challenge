...
class FlowDefaultEventClassifier {
  classify = (flowEvent) => {
    ...
    const firstHalfDuration =
      flowEvent.profile[halfIndex - 1].endEpoch -
      flowEvent.profile[0].startEpoch;
    const secondHalfDuration =
      flowEvent.profile[flowEvent.profile.length - 1].endEpoch -
      flowEvent.profile[halfIndex].startEpoch;
